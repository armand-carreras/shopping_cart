...
if("content" in document.createElement('template')){
  ...
  for(let i = 0; i<JSONArray.length ; i++){
    ...
    templateProd.content.querySelector(".flip-card-front").appendChild(image);
  
    templateProd.content.querySelectorAll(".nombre")[1].textContent=JSONArray[i].name;
    templateProd.content.querySelector(".category").textContent = JSONArray[i].category;
    templateProd.content.querySelector(".price").textContent = JSONArray[i].price + " €";
    templateProd.content.querySelector(".tags").textContent = JSONArray[i].tags;
    let clonProd = document.importNode(templateProd,true);
    cart.appendChild(clonProd.content);
...
document.querySelectorAll('.addButton').forEach(item => {
  item.addEventListener('click', event => {
    ...
    for(i = 0; i < asideList.length; i++){
      console.log(i);
      if(asideList[i].querySelector('.itemName').textContent === cajaPadre.querySelector('.nombre').textContent){
        
        parseInt(asideList[i].querySelector('.itemAmount').textContent)+1;
        
        asideList[i].querySelector('.totalPrice').textContent = cajaPadre.querySelector('.price').textContent * asideList[i].querySelector('.itemAmount').textContent;
